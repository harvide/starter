...
function mapDrizzleTypeToAuthType(column: any): FieldAttribute {
  ...
  if (['text', 'varchar'].includes(t)) {
    return { type: 'string', nullable: !column.notNull };
  }
  if (['boolean'].includes(t)) {
    return { type: 'boolean', nullable: !column.notNull };
  }
  if (['timestamp', 'date'].includes(t)) {
    return { type: 'date', nullable: !column.notNull };
  }
  if (['integer', 'numeric'].includes(t)) {
    return { type: 'number', nullable: !column.notNull };
  }
...
const additionalFields: Record<string, FieldAttribute> = Object.entries(columns)
  ...
  .reduce(
    (acc: Record<string, FieldAttribute>, [name, col]) => {
