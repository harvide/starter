...
*   **Leverage Existing Flows**: It's highly recommended to reuse the authentication flow logic provided in `apps/client/src/components/auth/login-form/flows.ts`. These functions abstract away the complex authentication interactions with the backend.
...
1.  **Create a new file**: For example, `apps/client/src/components/auth/login-form/variants/my-custom-login.tsx`.
...
3.  **Register the variant**: Update the `LoginFormVariants` map in `apps/client/src/components/auth/login-form/index.ts` to include your new component.
