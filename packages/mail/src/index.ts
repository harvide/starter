...
import type { MailBase } from './base';
...
export interface MailAdapter extends MailBase {
  ...
  sendTemplate(
    mailOptions: Omit<MailOptions, 'body'> & {
      template: string;
...
export function createMailAdapter(adapter: 'smtp' | 'resend'): MailAdapter {
  switch (adapter) {
    case 'smtp':
...
for (const [templateName, template] of Object.entries(templates)) {
  for (const [variantName, variantComponent] of Object.entries(
    template.variants
  )) {
    mail.registerTemplate(templateName, variantName, variantComponent);
  }
}
